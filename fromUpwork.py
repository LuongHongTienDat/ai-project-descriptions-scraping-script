...
driver.get(url)
rows = driver.find_elements(By.CSS_SELECTOR,'.job-tile > a')
linkLs = [row.get_attribute('href') for row in rows]
print(linkLs)
for link in linkLs: 
    driver.get(link)
    content = driver.find_element(By.CSS_SELECTOR,'.job-description div')
    df.loc[len(df.index)] = [url,content.text]

df.to_excel('data/from_upwork.xlsx')
